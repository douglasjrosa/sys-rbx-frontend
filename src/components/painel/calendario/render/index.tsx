  ...
  return (
    <>
      {calendarData.map((part: any, index: number) => {
        ...
        return (
        <Flex key={index} bg={'gray.700'} direction="column" w={part.length === 8? '58rem': part.length === 7? '58rem': part.length === 5? '42rem':part.length === 9? '65rem':part.length === 6? '44rem':'50rem'} alignItems={'center'} pt='3' pb='3'>
          <Box>
          ...
          <Box px={3} pt={'4'}>
            <Flex gap={'1px'}>
              {part?.map((item: any) => {
                ...
                return (
                  <Box key={item.id} w={'7rem'} minH={'6rem'} bg={'white'} p={1}>
