...
const discountDescriptionMap: Record<string, string> = {
  "Discount for quantity above 10 units per item.":
    "Desconto por quantidade acima de 10 unidades por item.",
  "Discount for order anticipation with at least 10 days notice.":
    "Desconto por antecipação de pedido com ao menos 10 dias de antecedência.",
  ...
  "Discount for straight Truck Full Load.":
    "Desconto por carga completa Truck.",
};
...
export const DynamicDiscounts: React.FC<DynamicDiscountsProps> = ({
  ...
  useEffect(() => {
    if (forceResetAll) {
      ...
      setManualDiscount(0);
    } else if (forceResetManual) {
      setManualDiscount(0);
    }
  }, [forceResetAll, forceResetManual]);
  ...
  const isPaymentOnOrderAvailable = useMemo(() => {
    ...
    if (
      lowerStr === "à vista" ||
      lowerStr === "a vista" ||
      lowerStr.startsWith("à vista") ||
      lowerStr.startsWith("a vista")
    ) {
      return true;
    }
  ...
  const handleManualDiscountChange: React.Dispatch<
    React.SetStateAction<number>
  > = (value) => {
    const numericValue =
      typeof value === "function" ? value(manualDiscount) : value;
  ...
  useEffect(() => {
    if (manualDiscount > maxManualDiscount) {
      const clampedValue = Math.max(
        0,
        Math.min(maxManualDiscount, maxManualDiscount),
      );
      setManualDiscount(clampedValue);
      if (onManualDiscountChange) {
        onManualDiscountChange(clampedValue);
  ...
  const regularDiscounts = useMemo(() => {
    ...
    return discounts.filter((discount) => {
      const discountRate =
        marginTable.discounts[
          discount.key as keyof typeof marginTable.discounts
        ];
      ...
      if (
        discount.key === "fullVucLoad" ||
        discount.key === "fullThreeQuarterLoad" ||
        discount.key === "straightTruckFullLoad"
      ) {
        return false;
      }
    ...
    marginTable,
    isAnticipationAvailable,
    isPaymentOnOrderAvailable,
    isFobFreightAvailable,
    hasQuantityDiscount,
  ...
  const totalDiscount = useMemo(() => {
    ...
    selectedDiscounts.forEach((discountKey) => {
      const discountRate =
        marginTable.discounts[
          discountKey as keyof typeof marginTable.discounts
        ];
  ...
  if (
    ...
    (regularDiscounts.length === 0 &&
      wholesaleLoadDiscounts.length === 0 &&
      !hasMontOrExpo)
  ) {
    return null;
  }
    ...
    <Box w="100%" mt={4} px={{ base: 2, md: 5 }} pb={4}>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Heading fontSize="1.3rem" fontWeight="bold">
          Descontos Disponíveis para Produtos com Tabela:
        </Heading>
      ...
      <Flex flexWrap="wrap" rowGap={5} columnGap={4} mb={2} alignItems="center">
        {regularDiscounts.map((discount) => {
          ...
          const descriptionPt =
            discountDescriptionMap[discount.description] ||
              ...
              >
                ...
                >
                  <FormLabel
                    ...
                    mb={0}
                    cursor="pointer"
                    onClick={() => handleDiscountToggle(discount.key)}
                  >
                    {namePt} ({percentage}%)
                  </FormLabel>
                </Checkbox>
              ...
              >
                ...
                >
                  <FormLabel
                    ...
                    onClick={
                      isEnabled
                        ? () => handleDiscountToggle(discount.key)
                        : undefined
                    }
                  >
                    {namePt} ({percentage}%)
