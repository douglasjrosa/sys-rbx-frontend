...
export default function EmpresaId () {
	...
	useEffect( () => {
		if ( DataEmp ) {
			const username = DataEmp?.attributes?.user?.data?.attributes?.username
			if ( username ) {
				if ( username !== session?.user.name ) {
					if ( session?.user.pemission !== 'Adm' ) {
						toast( {
							title: `O cliente ${ DataEmp?.attributes?.nome }`,
							description: `pertence ao vendedor(a) ${ username }`,
							status: 'warning',
							duration: 9000,
							isClosable: true,
							position: 'top-right',
	...
	}, [ DataEmp, session?.user.name, session?.user.pemission, toast ] )
