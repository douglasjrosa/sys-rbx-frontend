...
const FIELDS_QUERY =
  "fields[0]=nome&fields[1]=expiresIn&fields[2]=vendedorId&fields[3]=vendedor&populate[user][fields][0]=username";
...
async function processarEmpresa(
  ...
  const { nome, expiresIn, vendedor } = empresa.attributes;
  ...
  const vendedorFinal =
    update.vendedor !== undefined ? update.vendedor : vendedor;
...
export default async function CheckExpiration(
  ...
  try {
    ...
    if (processAll) {
      // Old behavior: process all pages (keep for legacy/local use)
      // Fetch first page to get total count
      const primeiraPagina = await axios.get(buildEmpresasUrl(1, pageSize), {
      ...
      console.log(
        `Processing all ${totalEmpresas} empresas across ${totalPaginas} pages`,
      );
      ...
      for (let paginaAtual = 1; paginaAtual <= totalPaginas; paginaAtual++) {
        const getEmpresas =
          paginaAtual === 1
            ? primeiraPagina
            : await axios.get(buildEmpresasUrl(paginaAtual, pageSize), {
                headers: buildHeaders(tokenString),
              });

        const empresas: Empresa[] = getEmpresas.data.data;
        const lotes = dividirEmLotes(empresas, batchSize);
      ...
      // Batch behavior: process only one page
      const response = await axios.get(buildEmpresasUrl(page, pageSize), {
        headers: buildHeaders(tokenString),
      });
