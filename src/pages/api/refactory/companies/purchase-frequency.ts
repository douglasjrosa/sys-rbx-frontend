		...
		for ( const empresa of allCompanies ) {
			try {
				...
				const wonBusinesses = businesses
					.filter( ( b: any ) => {
						const dateConclucao = b.attributes?.date_conclucao || b.date_conclucao
				...
				if ( averageDays === null || averageDays > 180 ) {
					...
					expiresIn = formatISO( addDays( baseDate, 365 ), { representation: 'date' } )
					...
					const baseDate = lastBusinessDate || currentDate
					expiresIn = formatISO( addDays( baseDate, 200 ), { representation: 'date' } )
				} else {
					// Mensalmente: expiresIn = 40 days after last business or current date
					purchaseFrequency = 'Mensalmente'
					const baseDate = lastBusinessDate || currentDate
					expiresIn = formatISO( addDays( baseDate, 40 ), { representation: 'date' } )
				}
				...
				results.details.push( {
					...
					averageDays: averageDays !== null ? Math.round( averageDays * 100 ) / 100 : null,
					purchaseFrequency,
					expiresIn,
